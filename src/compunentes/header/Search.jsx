  ...
  const Search = (props) => {
    ...
    function SearchTitle(e) {
      ...
      setTime = setTimeout(() => {
        axios
          .get(
            `http://localhost:3150/user/fillter?productName=${e}`
          )
          .then(function (res) {
            ...
            if(dataSearch.length>0){
              ...
              setPost([{productName:'không có kết quả nào phù hợp, mời bạn nhập lại !!!'}]);
            }
            clearTimeout(setTime);
          })
          .catch((err) => {
          ...
          <h3 className="header_search-history-heading">
            ...
            <ul className="header_search-history-heading-text-list">
                {post.length > 0? post.map((val)=>{ 
                  return ( <li onClick={(e)=>{movePageToProduct(e)}} className="header_search-history-heading-text-list-item"> {val.productName}</li> )               
                }): <li  onClick={(e)=>{movePageToProduct(e)}} className="header_search-history-heading-text-list-item"> mời bạn nhập ..........</li>}
